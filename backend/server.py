...
from activity_tracker import add_activity
...
def init_db():
    with db_lock:
        ...
        for stmt in (
            ...
            "ALTER TABLE users ADD COLUMN profile_media_id INTEGER",
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _do_GET(self):
        ...
        if path=='/api/community-activities':
            ...
            with db_lock:
                con = db()
                rows = con.execute(
                    'SELECT id, username, action_type, action_text, link, emoji, created_at FROM community_activities ORDER BY id DESC LIMIT ?',
                    (limit,)
                ).fetchall()
                con.close()
            ...
            for r in rows:
                activities.append({
                    ...
                    'timeAgo': max(0, now - r['created_at'])
                })
        ...
        if path=='/api/skills':
            ...
            with db_lock:
                con=db()
                rows=con.execute("SELECT id,name,whatsapp,category,description,status,created_at,IFNULL(owner_username,'') AS owner_username,IFNULL(owner_id,0) AS owner_id FROM skills WHERE status!='rejected' ORDER BY (status='verified') DESC, id DESC LIMIT 200").fetchall()
                out=[]
                for r in rows:
                    ow=(r['owner_username'] if 'owner_username' in r.keys() and r['owner_username'] else '')
                    try:
                        if 'owner_id' in r.keys() and r['owner_id']:
                            u=con.execute('SELECT username FROM users WHERE id=?',(r['owner_id'],)).fetchone()
                            if u and u['username']: ow=u['username']
                    except Exception: pass
                    out.append({'id':r['id'],'name':r['name'],'whatsapp':r['whatsapp'],'category':r['category'],'description':r['description'],'status':r['status'],'createdAt':r['created_at'],'owner':ow})
                con.close()
        ...
        if path == '/api/marketplace':
            if self.command == 'GET':
                # Get all listings
                con = db()
                rows = con.execute('SELECT id, title, category, price, description, contact, seller, created_at FROM marketplace ORDER BY id DESC LIMIT 200').fetchall()
                con.close()
                send_json(self, {'listings': [dict(r) for r in rows]})
                return
        if self.command == 'POST':
                ...
                if not title or not price or not description or not contact or not seller:
                    send_json(self, {'error': 'All fields are required'}, 400)
                    return
                con = db()
                cur = con.execute(
                    'INSERT INTO marketplace (title, category, price, description, contact, seller, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    (title, category, price, description, contact, seller, int(time.time()))
                )
                con.commit()
                con.close()
                send_json(self, {'success': True, 'id': cur.lastrowid})
                return
        if path == '/api/marketplace':
            if self.command == 'GET':
                # Get all listings
    ...
    def _do_POST(self):
        ...
        if path in ('/api/media/upload', '/api/profile/photo'):
            data={}
        else:
        ...
        if path=='/api/join':
            ...
            with db_lock:
                ...
                try:
                    con.execute(
                        'INSERT INTO users(username,client_id,session_token,identity_expires,last_seen,username_changed_at,username_change_available_at,recovery_code_hash,recovery_code_set_at) VALUES(?,?,?,?,?,?,?,?,?)',
                        (username,client_id,token,exp,now,now,now+IDENTITY_DAYS*86400,code_hash,now)
                    )
                    uid=con.execute('SELECT last_insert_rowid()').fetchone()[0]

                except sqlite3.IntegrityError as e:
                    if "users.client_id" in str(e):
                        new_client_id = client_id + "-" + secrets.token_hex(4)
                        con.execute(
                            'INSERT INTO users(username,client_id,session_token,identity_expires,last_seen,username_changed_at,username_change_available_at,recovery_code_hash,recovery_code_set_at) VALUES(?,?,?,?,?,?,?,?,?)',
                            (username,new_client_id,token,exp,now,now,now+IDENTITY_DAYS*86400,code_hash,now)
                        )
                        uid = con.execute('SELECT last_insert_rowid()').fetchone()[0]
                    else:
                        con.close()
                        send_json(self,{'error':'Registration database conflict','details':str(e)},409)
                        return
                ...
                add_activity(
                    ...
                    username,
                    "signup",
                    "joined the FamilyHub community",
                    "/community",
                    "🎉"
                )
        ...
        if path=='/api/admin/reports/resolve':
            ...
            with db_lock:
                ...
                con.commit(); con.close(); send_json(self,{'ok':True}); return
        if path=='/api/skills':
            # Skill Hub registration — requires login. The listing's display
            # name is always the account's own community username (not a
            # free-text field), so "Message" buttons on the directory can
            # route straight into that user's real in-platform chat thread.
            con=db(); user=auth(con,data,self)
            if not user: con.close(); send_json(self,{'error':'Please log in first.'},401); return
            category=str(data.get('category','')).strip()[:80]
            description=str(data.get('description','')).strip()[:600]
            if not category or not description:
                con.close(); send_json(self,{'error':'Please fill in your skill/category and description.'},400); return
            now=int(time.time())
            ref='SKL-'+secrets.token_hex(3).upper()
            while con.execute('SELECT 1 FROM skills WHERE ref_code=?',(ref,)).fetchone():
                ref='SKL-'+secrets.token_hex(3).upper()
            con.execute('INSERT INTO skills(name,whatsapp,category,description,status,ref_code,fee_paid,owner_id,owner_username,created_at) VALUES(?,?,?,?,?,?,0,?,?,?)',
                        (user['username'],'',category,description,'pending',ref,user['id'],user['username'],now))
            con.commit(); con.close()
            send_json(self,{'ok':True,'refCode':ref,
                             'message':'Thanks — your skill listing is live. Community members can now message you directly.'}); return
        if path=='/api/skills/thread':
        ...
        if path=='/api/profile/photo':
            ...
            try:
                ...
                con.close(); send_json(self,{'error':str(e)},400); return
            safe_name=re.sub(r'[^A-Za-z0-9._-]+','_',Path(filename).name)[:120] or 'profile'
            mime=mime.split(';',1)[0].strip().lower()
            if mime not in ALLOWED_IMAGE_TYPES:
                con.close(); send_json(self,{'error':'Unsupported image type. Use JPG, PNG, or WebP.'},415); return
            if len(data_bytes)==0 or len(data_bytes)>MAX_IMAGE_BYTES:
                con.close(); send_json(self,{'error':'Image is empty or exceeds the current size limit.'},413); return
            now=int(time.time()); storage=secrets.token_hex(16)+'_'+safe_name
            old_media_id=user['profile_media_id']
            try:
                (MEDIA_DIR/storage).write_bytes(data_bytes)
                cur=con.execute('INSERT INTO media(owner_id,kind,mime_type,original_name,storage_name,size_bytes,created_at,expires_at) VALUES(?,?,?,?,?,?,?,?)',(user['id'],'image',mime,safe_name,storage,len(data_bytes),now,now+50*365*86400))
                mid=cur.lastrowid
                con.execute('UPDATE users SET profile_media_id=? WHERE id=?',(mid,user['id']))
                con.commit()
                if old_media_id:
                    oldm=con.execute('SELECT storage_name FROM media WHERE id=?',(old_media_id,)).fetchone()
                    if oldm:
                        try: (MEDIA_DIR/oldm['storage_name']).unlink(missing_ok=True)
                        except Exception: pass
                        con.execute('DELETE FROM media WHERE id=?',(old_media_id,)); con.commit()
                con.close(); send_json(self,{'ok':True,'profilePhotoId':mid}); return
            except Exception:
                try: (MEDIA_DIR/storage).unlink(missing_ok=True)
                except Exception: pass
                con.close(); send_json(self,{'error':'Unable to store photo.'},500); return
        ...
        with db_lock:
            ...
            if path=='/api/profile':
                ...
                location=str(data.get('location',user['location'] or '')).strip()[:100]
                sex=str(data.get('sex',user['sex'] or '')).strip()[:20]
                if sex and sex not in ('Male','Female','Prefer not to say'):
                    con.close(); send_json(self,{'error':'Invalid sex value.'},400); return
                country=str(data.get('country',user['country'] or '')).strip()[:60]
                state=str(data.get('state',user['state'] or '')).strip()[:60]
                whatsapp=str(data.get('whatsapp',user['whatsapp'] or '')).strip()[:32]
                social_link=str(data.get('socialLink',user['social_link'] or '')).strip()[:200]
                hide_photo=bool(data.get('hidePhoto',user['hide_photo']))
                ...
                if requested_username and requested_username.lower()!=user['username'].lower():
                    ...
                    con.execute('UPDATE users SET username=?,username_changed_at=?,username_change_available_at=?,hidden=?,location=?,sex=?,country=?,state=?,whatsapp=?,social_link=?,hide_photo=?,last_seen=?,identity_expires=? WHERE id=?',(requested_username,now,now+IDENTITY_DAYS*86400,1 if hidden else 0,location,sex,country,state,whatsapp,social_link,1 if hide_photo else 0,now,now+IDENTITY_DAYS*86400,user['id']))
                    con.commit(); con.close(); send_json(self,{'ok':True,'username':requested_username,'hidden':hidden,'location':location,'sex':sex,'country':country,'state':state,'whatsapp':whatsapp,'socialLink':social_link,'hidePhoto':hide_photo,'usernameChangedAt':now,'usernameChangeAvailableAt':now+IDENTITY_DAYS*86400,'identityExpires':now+IDENTITY_DAYS*86400,'usernameChanged':True}); return
                con.execute('UPDATE users SET hidden=?,location=?,sex=?,country=?,state=?,whatsapp=?,social_link=?,hide_photo=?,last_seen=? WHERE id=?',(1 if hidden else 0,location,sex,country,state,whatsapp,social_link,1 if hide_photo else 0,now,user['id'])); con.commit(); con.close(); send_json(self,{'ok':True,'username':user['username'],'hidden':hidden,'location':location,'sex':sex,'country':country,'state':state,'whatsapp':whatsapp,'socialLink':social_link,'hidePhoto':hide_photo,'usernameChangeAvailableAt':int(user['username_change_available_at'] or 0),'usernameChanged':False}); return
            ...
            if path=='/api/community':
                ...
                add_activity(
                    ...
                    user['username'],
                    'community_post',
                    'posted a new community discussion',
                    '/community-chat.html',
                    '💬'
                )
